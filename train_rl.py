...
from snek_env_torch import TorchSnekBatch
...
def main():
    ...
    parser.add_argument("--eps-reset", type=float, default=0.7)
    ...
    parser.add_argument("--eps-exp-k", type=float, default=12.0)
...
def _main(args, out_dir: Path, error_log: Path):
    ...
    env = TorchSnekBatch(args.n_envs, config, target_h, device, random_start)
    ...
    while True:
        ...
        if now - last_log_time >= args.log_interval_sec:
            ...
            if recent_rewards:
                ...
                mean_max_len = np.mean(recent_max_lens) if recent_max_lens else 0.0
            else:
                mean_reward = float(np.mean(episode_rewards)) if episode_rewards.size else 0.0
                mean_len = float(np.mean(episode_lengths)) if episode_lengths.size else 0.0
                mean_max_len = float(np.mean(episode_max_len)) if episode_max_len.size else 0.0
            if recent_max_lens:
                best_train_max_len = max(best_train_max_len, max(recent_max_lens))
            mean_reward_long = np.mean(recent_rewards_long) if recent_rewards_long else 0.0
            mean_len_long = np.mean(recent_lengths_long) if recent_lengths_long else 0.0
            mean_max_len_long = np.mean(recent_max_lens_long) if recent_max_lens_long else 0.0
        ...
        if plot_sock is not None and (now - last_stream_time) >= args.plot_stream_interval_sec:
            mean_reward_live = float(np.mean(recent_rewards)) if recent_rewards else 0.0
            mean_len_live = float(np.mean(recent_lengths)) if recent_lengths else 0.0
            mean_max_len_live = float(np.mean(recent_max_lens)) if recent_max_lens else 0.0
            mean_reward_long_live = float(np.mean(recent_rewards_long)) if recent_rewards_long else 0.0
            mean_len_long_live = float(np.mean(recent_lengths_long)) if recent_lengths_long else 0.0
            mean_max_len_long_live = float(np.mean(recent_max_lens_long)) if recent_max_lens_long else 0.0
